...
public abstract class Hero implements MinionListener {
	...
	public void playMinion(Minion m) throws NotYourTurnException, NotEnoughManaException, FullFieldException {
		...
		field.add(m);
		this.currentManaCrystals -= ((Card) m).getManaCost();
		hand.remove(m);
	...
	public void castSpell(FieldSpell s) throws NotYourTurnException, NotEnoughManaException {
		...
		if (this instanceof Mage) {
			for (int i = 0; i < this.field.size(); i++) {
				if (field.get(i).getName().equals("Kalycgos")) {
					Card c = (Card) s;
	...
	public Card drawCard() throws FullHandException, CloneNotSupportedException {
		if (hand.size() == 10 && deck.size() != 0)
			throw new FullHandException("Your hand is full!!",deck.remove(0));
		for (int i = 0; i < field.size(); i++) {
			if (field.get(i).getName().equals("Chromaggus")) {
				Card tmp = deck.get(0).clone();
				if (hand.size() < 9) {
					hand.add(tmp);
				}
				break;
	...
	abstract public String getHeroPower();
